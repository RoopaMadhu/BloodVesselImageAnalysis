...
%% edited by Roopa Madhu - 02.02.2017
%% Main function
...
for k = 1:length(n)
    ni = n(k);
    imgData  = ReadAnImageData(imgList(ni), filename);
    DataExtracted(k).imgStruct= ReconstructImage(imgList(ni), imgData);
    DataExtracted(k).imgStruct.Name            = dat{ni,1};
    DataExtracted(k).imgStruct.Type            = dat{ni,3};
    DataExtracted(k).imgStruct.NumberOfChannel = dat{ni,2};
    DataExtracted(k).imgStruct.Size            = dat{ni,4}; 
    clear imgData;
end
imgout=DataExtracted;
return
